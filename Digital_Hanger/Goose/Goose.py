...
def main():
    ...
    mission = simple_mission_setup(analyses)
...
def vehicle_setup():
    ...
    # mass properties 
    vehicle.mass_properties.max_takeoff               =   14 * Units.pounds  # kg
    ...
    vehicle.mass_properties.takeoff                   =   14 * Units.pounds  # kg
    vehicle.mass_properties.max_zero_fuel             =   12 * Units.pounds  # kg
    vehicle.mass_properties.max_payload               =   1 * Units.pounds  # kg
    vehicle.mass_properties.max_fuel                  =   2 * Units.pounds  # kg
    vehicle.mass_properties.payload                   =   0 # kg
    ...
    vehicle.flight_envelope.design_range           = 5 * Units.mile
    vehicle.flight_envelope.design_cruise_altitude = 3000 * Units.feet
    vehicle.flight_envelope.design_mach_number     = 0.1
    vehicle.flight_envelope.design_dynamic_pressure = 0
    

    # basic parameters
    vehicle.reference_area         = .49054
    ...
    wing.tag                               = 'main_wing'
    wing.areas.reference                   = .49054
    wing.chords.root                       = .253
    wing.chords.tip                        = .15350
    wing.sweeps.leading_edge               = -15.0 * Units.deg
    wing.thickness_to_chord                = 0.100
    wing.chords.mean_aerodynamic           = .277 * Units.meter 
    wing.taper                             = wing.chords.tip / wing.chords.root
    wing.dihedral                          = -10 * Units.deg
    wing.spans.projected                   = 1.711
    wing.aspect_ratio                      = 5.97
    wing.origin                            = [[.471,0.0,0.020]]
    wing.vertical                          = False
    ...
    segment.sweeps.leading_edge  = -3.07 * Units.degrees
    ...
    segment.twist                 = 0
    segment.root_chord_percent    = 1.107
    segment.thickness_to_chord    = 0.1
    segment.dihedral_outboard     =  -9.9* Units.degrees
    segment.sweeps.leading_edge  =  -25.28* Units.degrees
    wing.append_segment(segment)

    segment = RCAIDE.Library.Components.Wings.Segments.Segment()
    segment.tag                   = 'segment_2'
    segment.percent_span_location = 0.511
    segment.twist                 = 0
    segment.root_chord_percent    = 1.344
    segment.thickness_to_chord    = 0.1
    segment.dihedral_outboard     =  -15.46* Units.degrees
    segment.sweeps.leading_edge  =  -16.52* Units.degrees
    wing.append_segment(segment)
    ...
    segment.root_chord_percent    = 1.107
    segment.thickness_to_chord    = 0.1
    segment.dihedral_outboard     = -20.82 * Units.degrees
    segment.sweeps.quarter_chord  = .63 * Units.degrees
    wing.append_segment(segment)       
    ...
    aileron.tag                   = 'aileron'
    aileron.span_fraction_start   = 0.25
    aileron.span_fraction_end     = 0.5
    ...
    #  Fuel Tank & Fuel
    #------------------------------------------------------------------------------------------------------------------------------------   
    fuel_tank                                   = RCAIDE.Library.Components.Powertrain.Sources.Fuel_Tanks.Integral_Tank(vehicle.wings.main_wing)  # This has changed
    ...
    turbofan.design_thrust                          = 30* Units.N 
...
#   Define the Mission
# ----------------------------------------------------------------------
def simple_mission_setup(analyses):
    ...
    segment.air_speed                                                           = 146 * Units['knots']
    ...
    segment.assigned_control_variables.throttle.assigned_propulsors             = [['inner_starboard_propulsor', 'inner_port_propulsor', 'outer_starboard_propulsor', 'outer_port_propulsor']]
