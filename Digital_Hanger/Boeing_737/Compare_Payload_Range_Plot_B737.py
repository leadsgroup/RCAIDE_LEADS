...
payload_range = {
    "Airport Planning Manual": {
        "range": [0, 2010, 4000, 4750],
        "payload": (np.array([46500, 46500, 23500, 0])).tolist(),
        "payload + oew": (np.array([138000, 138000, 115000, 91500])).tolist(),
   },
    "Boeing 737-800": {
        "range": (np.array([   0.        , 2114.59 , 4195.12 , 4749.46]) ).tolist(),
        "payload": (np.array([21000.00,21000.00 , 11130.43 ,0.]) / Units.lbs).tolist(),
...
legend_elements = [
    ...
    plt.Line2D([0], [0], color=colors[1], lw=2.5, marker='o', markersize=6, label='Boeing 737-800 RCAIDE')
]
