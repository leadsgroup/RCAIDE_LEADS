...
def Landing_Gear_Validation(PP): 
 ...
 # define operating conditions                                            
    ...
    # Apply original truncation to keep behavior identical
    track_df = track_df.head(50).reset_index(drop=True)
    
    # NEW: Upsample the track to eliminate numerical integration jitter
    track_df = fine_interpolate_track(track_df, max_dist_m=20.0)
    
    # Filter to points equivalent to original index > 10
    track_df = track_df[track_df['float_index'] > 10.0].reset_index(drop=True)
    ...
    df = downsample_spatial_grid(df, stride_factor=8)
    ...
    # Aircraft speed from upsampled track
    U_flight = track_df['Ground Speed (kts)'].tolist()
    U_flight = [x * 0.5144 for x in U_flight] 

    settings = Data()
    settings.center_frequencies = np.pad(frequency, (5, 0), mode='constant')
    altflight = []
    
    for index, track_point in track_df.iterrows(): 
        ...
        d_seg_m = np.maximum(d_seg_m, 0.3) 
        ...
        for i in range(num_receptors):
            ...
            attenuated_spectrum = total_spectrum - (att_dB * 1.1) - LADJ_dB[i] 
    ...
    # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    # Plotting the results after the loop finishes
    # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    
    # Extract original points for accurate dot plotting
    orig_pts = track_df[(track_df['is_original'] == True) & (track_df['float_index'] > 17.0)]
    positionsx = orig_pts['Latitude (deg)'].tolist()
    positionsy = orig_pts['Longitude (deg)'].tolist()
    tr = orig_pts['Noise Thrust per Engine (lbs)'].round().tolist()
...
def fine_interpolate_track(df, max_dist_m=20.0):
    """
    Interpolates the track dynamically to ensure the evaluation nodes 
    are spaced closely enough to eliminate numerical integration jitter.
    """
    R_earth = 6371000.0
    new_rows = []
    ...
    for i in range(len(df) - 1):
        ...
        lat1, lon1 = np.radians(pt1['Latitude (deg)']), np.radians(pt1['Longitude (deg)'])
        lat2, lon2 = np.radians(pt2['Latitude (deg)']), np.radians(pt2['Longitude (deg)'])
        
        dy = (lat2 - lat1) * R_earth
        dx = (lon2 - lon1) * R_earth * np.cos(lat1)
        dz = (pt2['Altitude MSL (ft)'] - pt1['Altitude MSL (ft)']) * 0.3048
        dist = np.sqrt(dx**2 + dy**2 + dz**2)
        
        num_steps = max(1, int(np.ceil(dist / max_dist_m)))
        
        for step in range(num_steps):
            frac = step / float(num_steps)
            new_pt = {}
            for col in df.columns:
                # FIX: Explicitly exclude boolean columns to prevent numpy subtraction errors
                if pd.api.types.is_numeric_dtype(df[col]) and not pd.api.types.is_bool_dtype(df[col]):
                    new_pt[col] = pt1[col] + frac * (pt2[col] - pt1[col])
                else:
                    new_pt[col] = pt1[col] 
            new_pt['float_index'] = i + frac
            new_pt['is_original'] = (step == 0) 
            new_rows.append(new_pt)
            
    last_pt = df.iloc[-1].to_dict()
    last_pt['float_index'] = float(len(df) - 1)
    last_pt['is_original'] = True
    new_rows.append(last_pt)
