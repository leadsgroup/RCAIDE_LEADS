...
def check_compound_surface_mirroring(vehicle, control_surface, primary_coeff_name, secondary_coeff_name):
    ...
    # Ruddervator's yaw (secondary) authority is inherently weaker than its pitch (primary)
    # authority for a dihedral V-tail (a known real-world limitation of ruddervators versus a
    # dedicated rudder), which makes its already-small cross-term a larger fraction of that
    # weak secondary response — hence a looser relative threshold than a tighter absolute check
    # would need.
