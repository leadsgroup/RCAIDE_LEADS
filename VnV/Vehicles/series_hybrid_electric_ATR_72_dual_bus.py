# series_hybrid_electric_ATR_72_dual_bus.py
#
# Created: 2026, network robustness regression
#
# Same airframe as series_hybrid_electric_ATR_72.py, but with a deliberately
# realistic dual-electrical-bus architecture instead of a single mixed bus:
#
#   - Main propulsive bus:   providers = 2x turboelectric_generator ONLY (no battery)
#   - Essential/standby bus: provider  = a small dedicated battery pack ONLY,
#                             consumer  = avionics
#
# The two buses are completely isolated from each other. Each bus is
# individually unambiguous (main bus: generator-only -> psi should resolve to
# 0; essential bus: battery-only -> psi should resolve to 1), but a
# vehicle-wide (rather than per-distributor) existence check for "are there
# batteries AND other electrical providers anywhere in the network" sees both
# a battery (on the essential bus) and generators (on the main bus) and
# concludes the whole vehicle is ambiguous -- which is wrong for both buses
# individually. This mirrors a real aircraft's standby-battery-bus
# architecture (kept electrically independent from the main generator bus for
# certification reasons), not a synthetic edge case.
...
def vehicle_setup():
    ...
    #------------------------------------------------------------------------------------------------------------------------------------
    # Main propulsive bus -- fed ONLY by the two turboelectric generators, no battery
    #------------------------------------------------------------------------------------------------------------------------------------
    bus                              = RCAIDE.Library.Components.Powertrain.Distributors.Electrical_Bus()
    bus.tag                          = 'propulsive_bus'
    # A generator-only bus has no source to derive voltage from automatically
    # (see Electrical_Bus.initialize()), so it must be set explicitly here.
    bus.design_voltage               = 800.0
    ...
    #------------------------------------------------------------------------------------------------------------------------------------
    # Assign propulsive bus to propulsors
    ...
    generator.nominal_voltage         = bus.design_voltage
    ...
    # Two identical generators are the SOLE providers on the propulsive bus --
    # no battery here. This bus is individually unambiguous (psi should
    # resolve to 0.0, all-generator).
    ...
    #------------------------------------------------------------------------------------------------------------------------------------
    # Essential/standby bus -- fed ONLY by a small dedicated battery, powers
    # ONLY avionics. Completely isolated from the propulsive bus above,
    # mirroring a real aircraft's standby-battery architecture.
    #------------------------------------------------------------------------------------------------------------------------------------
    essential_bus                    = RCAIDE.Library.Components.Powertrain.Distributors.Electrical_Bus()
    essential_bus.tag                = 'essential_bus'

    essential_battery_pack                                        = RCAIDE.Library.Components.Powertrain.Sources.Batteries.Battery_Pack()
    essential_battery_pack.tag                                    = 'essential_battery_pack'
    essential_bat_module                                          = RCAIDE.Library.Components.Powertrain.Sources.Batteries.Modules.Lithium_Ion_NMC()
    essential_bat_module.electrical_configuration.series          = 20
    essential_bat_module.electrical_configuration.parallel        = 4
    essential_bat_module.cell.nominal_capacity                    = 3.8
    essential_bat_module.geometric_configuration.normal_count     = 4
    essential_bat_module.geometric_configuration.parallel_count   = 4

    for _ in range(2):
        essential_bat_copy = deepcopy(essential_bat_module)
        essential_battery_pack.append_module(essential_bat_copy)

    essential_battery_pack.battery_module_electric_configuration = 'Series'
    essential_battery_pack.assigned_distributors                 = [[essential_bus.tag]]
    net.sources.append(essential_battery_pack)
    essential_battery_pack.initialize(net)

    #------------------------------------------------------------------------------------------------------------------------------------
    # Avionics -- the essential bus's sole consumer
    #------------------------------------------------------------------------------------------------------------------------------------
    avionics                     = RCAIDE.Library.Components.Powertrain.Systems.Avionics()
    avionics.power_draw          = 30. # Watts
    avionics.assigned_distributors = [[essential_bus.tag]]
    net.systems.append(avionics)

    net.distributors.append(essential_bus)
