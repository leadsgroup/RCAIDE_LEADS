...
def main():
    ...
    escort_mission     = None # escort_mission_setup(analyses)
    dogfight_mission   = None # dogfight_mission_setup(analyses)
    long_range_mission = None # long_range_mission_setup(analyses)
    long_range_mission_h2 = None # long_range_H2_mission_setup(analyses)
    long_range_mission_dual_fuel = long_range_dual_fuel_mission_setup(analyses)
    nominal_mission    = None # mission_setup(analyses)\

    missions = missions_setup(nominal_mission, long_range_mission, long_range_mission_h2, long_range_mission_dual_fuel, escort_mission, dogfight_mission) 
    ...
    # long_range_mission_h2_results = missions.long_range_mission_h2.evaluate() 
    long_range_mission_dual_fuel_results = missions.long_range_mission_dual_fuel.evaluate() 
    # nominal_mission_results = missions.nominal_mission.evaluate()
    
    # Step 6 plot results 
    # plot_mission(nominal_mission_results)
    # plot_mission(long_range_mission_results)
    # plot_mission(long_range_mission_h2_results)
    plot_mission(long_range_mission_dual_fuel_results)
...
def vehicle_setup(drop_tank): 
    ...
    vehicle.mass_properties.max_fuel                  = 11900  #external tanks 8165 * Units.kilogram  11900 
    vehicle.mass_properties.fuel                      =  11900  #external tanks 8165 * Units.kilogram    # 57500 *Units.kilogram
    ...
    wing.taper                   = 0.349
    wing.spans.projected         = 6.25951 * Units.meter # Note that this assumes the tail meets int he middle (but aerodynamically it won't. This will affect weight calcualtions)
    ...
    wing.areas.exposed           = 15.00 * Units['meters**2']    # Exposed area of the horizontal tail
    wing.areas.wetted            = 35.00 * Units['meters**2']     # Wetted area of the horizontal tail
    wing.twists.root             = 0.0 * Units.degrees
    ...
    turbofan1.design_altitude                    = 0*Units.ft #60000*Units.ft             
    turbofan1.design_mach_number                 = 0.1 #2.25                    
    turbofan1.design_thrust                      = 26000 * Units.lbf#8300. * Units.lbf  
...
def configs_setup(vehicle):
    ...
    for propulsor in config.networks.fuel.propulsors:
        ...
        propulsor.afterburner.fuel_data               = RCAIDE.Library.Attributes.Propellants.Liquid_Hydrogen() 
   
    config.networks.fuel.fuel_lines.fuel_line.fuel_tanks.wing_fuel_tank.fuel = RCAIDE.Library.Attributes.Propellants.Liquid_Hydrogen()  
    
    if vehicle.Drop_Tank:
        config.networks.fuel.fuel_lines.fuel_line.fuel_tanks.tank_starboard.fuel = RCAIDE.Library.Attributes.Propellants.Liquid_Hydrogen()
        config.networks.fuel.fuel_lines.fuel_line.fuel_tanks.tank_port.fuel      = RCAIDE.Library.Attributes.Propellants.Liquid_Hydrogen()   
    ...
    # ------------------------------------------------------------------
    #  Afterburner Cruise Configuration
    # ------------------------------------------------------------------ 
    config     = RCAIDE.Library.Components.Configs.Config(base_config)
    config.tag = 'cruise_afterburner'  
    for propulsor in config.networks.fuel.propulsors:
        propulsor.afterburner_active = True 
    configs.append(config) 

    # ------------------------------------------------------------------
    #   Cruise Configuration - H2
    # ------------------------------------------------------------------ 
    config     = RCAIDE.Library.Components.Configs.Config(base_config)
    config.tag = 'cruise_h2'  
    for propulsor in config.networks.fuel.propulsors:
        propulsor.combustor.fuel_data                 = RCAIDE.Library.Attributes.Propellants.Liquid_Hydrogen()
        propulsor.afterburner.fuel_data               = RCAIDE.Library.Attributes.Propellants.Liquid_Hydrogen() 
    ...
    # ------------------------------------------------------------------
    #   Descent Configuration - H2
    # ------------------------------------------------------------------

    config                                = RCAIDE.Library.Components.Configs.Config(base_config)
    config.tag                            = 'descent_h2'  
    for propulsor in config.networks.fuel.propulsors:
        propulsor.combustor.fuel_data                 = RCAIDE.Library.Attributes.Propellants.Liquid_Hydrogen()
        propulsor.afterburner.fuel_data               = RCAIDE.Library.Attributes.Propellants.Liquid_Hydrogen() 
...
def mission_setup(analyses):
    ...
    segment.air_speed_end     =  300.  * Units.kts
    ...
    segment.air_speed_start     =  300.  * Units.kts
    segment.air_speed_end       =  900.  * Units.kts
    ...
    segment.air_speed                                     = 900.  * Units.kts
...
def long_range_H2_mission_setup(analyses): 

    # ------------------------------------------------------------------
    #   Initialize the Mission
    # ------------------------------------------------------------------
    mission = RCAIDE.Framework.Mission.Sequential_Segments()
    mission.tag = 'long_range_mission_h2'
    ...
    segment = Segments.Climb.Constant_Speed_Constant_Rate(base_segment)
    segment.tag = "climb_1"
    segment.analyses.extend( analyses.climb_h2 )
    segment.altitude_start = 0.0   * Units.km
    segment.altitude_end   = 4000. * Units.ft
    segment.airpseed       = 250.  * Units.kts
    segment.climb_rate     = 4000. * Units['ft/min']
    ...
    segment.analyses.extend( analyses.climb_h2 )
    segment.altitude_end   = 33000. * Units.ft
    segment.air_speed_end  = 500.   * Units.kts
    segment.climb_rate     = 2000.  * Units['ft/min']
    segment.true_course    = 0.0 * Units.deg
    ...
    segment = Segments.Cruise.Constant_Speed_Constant_Altitude(base_segment)
    segment.tag = "cruise"
    segment.analyses.extend( analyses.cruise_h2)
    segment.air_speed       = 500. * Units.kts
    segment.distance        = 1400 * Units.nmi
    ...
    segment = Segments.Descent.Linear_Mach_Constant_Rate(base_segment)
    segment.tag = "descent_1"
    segment.analyses.extend( analyses.descent_h2 )
    segment.altitude_end     = 4000. * Units.ft
    segment.mach_number_end  = 0.53
    segment.descent_rate     = 2000. * Units['ft/min']
    segment.true_course      = 0.0 * Units.deg
...
def long_range_dual_fuel_mission_setup(analyses): 

    # ------------------------------------------------------------------
    #   Initialize the Mission
    # ------------------------------------------------------------------
    mission = RCAIDE.Framework.Mission.Sequential_Segments()
    mission.tag = 'long_range_mission_dual_fuel'
...
def escort_mission_setup(analyses):
    ...
    segment.tag = "turn"
    segment.analyses.extend(analyses.climb)
    segment.altitude    = 33000. * Units.ft
    segment.air_speed   = 300. * Units.kts
    segment.turn_radius = 3000 * Units.feet
    segment.true_course = 0.0 * Units.degree
    segment.turn_angle  = 180.1132 * Units.degree
    ...
    segment.air_speed   = 500. * Units.kts
    segment.distance    = 500 * Units.nmi
    segment.true_course = 180.1132 * Units.deg
...
def dogfight_mission_setup(analyses):
    ...
    segment = Segments.Climb.Constant_Speed_Constant_Rate(base_segment)
    segment.tag = "climb_1"
    segment.analyses.extend(analyses.climb_afterburner)
    segment.altitude_start = 0.0 * Units.km
    segment.altitude_end = 4000. * Units.ft
    segment.air_speed = 350. * Units.kts
    segment.climb_rate = 5000. * Units['ft/min']
    segment.true_course = 0.0 * Units.deg
    ...
    segment.analyses.extend(analyses.climb_afterburner)
    segment.altitude_end = 8000. * Units.ft
    segment.air_speed_end = 800. * Units.kts
    segment.climb_rate = 2000. * Units['ft/min']
    segment.true_course = 0.0 * Units.deg
    ...
    segment = Segments.Cruise.Constant_Speed_Constant_Altitude(base_segment)
    segment.tag = "cruise_1"
    segment.analyses.extend(analyses.cruise_afterburner)
    segment.air_speed = 900. * Units.kts
    ...
    segment.air_speed = 800. * Units.kts
    segment.turn_radius = 8000 * Units.feet
    ...
    segment.air_speed = 500. * Units.kts
    segment.distance = 100 * Units.nmi
    segment.true_course = 183.814 * Units.deg
...
def missions_setup(nominal_mission, long_range_mission, long_range_mission_h2, long_range_mission_dual_fuel, escort_mission, dogfight_mission):
    ...
    # long_range_mission_h2.tag  = 'long_range_mission_h2'
    # missions.append(long_range_mission_h2) 

    long_range_mission_dual_fuel.tag  = 'long_range_mission_dual_fuel'
    missions.append(long_range_mission_dual_fuel) 

    # nominal_mission.tag  = 'nominal_mission'
    # missions.append(nominal_mission)    
