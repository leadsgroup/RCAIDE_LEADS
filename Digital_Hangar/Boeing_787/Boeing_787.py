...
def main(plot_results=True, plot_vehicle=True):
    ...
    export_vsp_vehicle(vehicle, 'Boeing_787_RCAIDE')
...
def vehicle_setup(vehicle_name = 'Boeing_787-8', passengers = 248) :
    ...
    #-------------------------------------------------------------------------------------------------------------------------
    # apu
    apu                      =  RCAIDE.Library.Components.Powertrain.Converters.Auxillary_Power_Unit()
    apu.mass_properties.mass = 238
    apu.origin               = [[54,0, 0]]
    net.converters.append(apu)  
    
    # Starboard Pump 
    starboard_pump                                 = RCAIDE.Library.Components.Powertrain.Converters.Pump()
    starboard_pump.working_fluid                   = RCAIDE.Library.Attributes.Propellants.Jet_A()  
    ...
    starboard_pump.origin                          = [[17.818, 10.000,-0.953]]
    net.converters.append(starboard_pump)  
   
    # Port Pump 
    port_pump                                      = deepcopy(starboard_pump) 
    port_pump.tag                                  = 'port_pump' 
    port_pump.origin                               = [[17.818, -10.000,-0.953]]
    net.converters.append(port_pump)  

    # Reserve Pump 
    reserve_pump                                  = deepcopy(starboard_pump) 
    reserve_pump.tag                              = 'reserve_pump' 
    reserve_pump.origin                           = [[17.818, 0,-0.953]]
    net.converters.append(reserve_pump)  

    # Assign propulsors to fuel line to network      
    fuel_line.assigned_converters =  [[starboard_pump.tag, port_pump.tag,reserve_pump.tag, apu.tag]]  
...
def configs_setup(vehicle):
    ...
    config.networks.fuel.propulsors['propulsor_1'].fan.rotation            = 2580.  
    config.networks.fuel.propulsors['propulsor_2'].fan.rotation            = 2580. 
    ...
    config.networks.fuel.propulsors['propulsor_1'].fan.rotation            = 2580.  
    config.networks.fuel.propulsors['propulsor_2'].fan.rotation            = 2580. 
    config.networks.fuel.propulsors['propulsor_1'].core_nozzle.noise_speed = 300.
    config.networks.fuel.propulsors['propulsor_2'].core_nozzle.noise_speed = 300.
    config.networks.fuel.propulsors['propulsor_1'].fan_nozzle.noise_speed  = 360.
    config.networks.fuel.propulsors['propulsor_2'].fan_nozzle.noise_speed  = 360. 
...
def noise_base_analysis(vehicle):
    ...
    noise.settings.noise_reduction_factors.SPL_dbA  = 0.05
...
def mission_setup(analyses):
    ...
    #base_segment.state.numerics.network_solver.method =  None
    #------------------------------------------------------------------------------------------------------------------------------------ 
    #  Takeoff Roll
    #------------------------------------------------------------------------------------------------------------------------------------ 
...
def simplfied_mission_setup(analyses):
    """This function defines the baseline mission that will be flown by the aircraft in order
    to compute performance."""
