...
def plot_battery_module_conditions(results,
                                  save_figure=False,
                                  show_legend=True,
                                  save_filename="Battery_Module_Conditions",
                                  file_type=".png",
                                  width=11, height=7):
    """
    Creates a six-panel plot showing battery module-level conditions throughout flight.
    ...
    save_filename : str, optional
        Base name of file for saved figure (default: "Battery_Module_Conditions")
    ...
    for network in results.segments[0].analyses.vehicle.networks:
        for b_i, source in enumerate(network.sources):
            ...
            for i, segment in enumerate(results.segments):
                ...
                ax_soc.plot(time, module_SOC, color=line_colors[i], marker=ps.markers[b_i],
                            linewidth=ps.line_width, label=label)
                ax_energy.plot(time, module_energy / Units.Wh, color=line_colors[i],
                               marker=ps.markers[b_i], linewidth=ps.line_width)
                ax_current.plot(time, module_current, color=line_colors[i],
                                marker=ps.markers[b_i], linewidth=ps.line_width)
                ax_power.plot(time, module_power, color=line_colors[i],
                              marker=ps.markers[b_i], linewidth=ps.line_width)
                ax_voltage.plot(time, module_volts, color=line_colors[i],
                                marker=ps.markers[b_i], linewidth=ps.line_width)
                ax_temp.plot(time, module_temp, color=line_colors[i],
                             marker=ps.markers[b_i], linewidth=ps.line_width)
    ...
    fig.suptitle('Battery Module Conditions')
    fig.tight_layout()
