...
def plot_cryogenic_tank_properties(results,
                             ...
                             width = 11, height = 7):
    """
    Creates a six-panel plot of cryogenic fuel tank conditions throughout flight:
    mass, temperature, and volume (fuel and ullage), pressure, vent rate, and
    boil-off rate, for every Cryogenic_Tank source in the vehicle.
    """
        ...
        for network in segment.analyses.vehicle.networks:
            for source in  network.sources:
                if isinstance(source, RCAIDE.Library.Components.Powertrain.Sources.Fuel_Tanks.Cryogenic_Tank):
                    ...
                    boil_off_rate      = tank_conditions.boil_off_flow_rate[:,0]

                    segment_tag  =  results.segments[i].tag
                    segment_name = segment_tag.replace('_', ' ')

                    tank_label = f"{source.tag}".replace("_"," ")
    ...
    # Title
    fig.suptitle("Cryogenic Tank Properties")
