...
class Cabin(Component):
    ...
    def __defaults__(self):
        ...
        self.segments_bounding_cabin   = []
        self.filled_seats_arrangement  = 'ascending'
    ...
    def append_segment(self, segment):
        ...
        ----------
        segment : Data
            Cabin segment to be added
        """
        # Assert database type
        if not isinstance(segment, RCAIDE.Library.Components.Fuselages.Cabins.Segments.Segment):
        ...
        for fuselage in vehicle.fuselages:
            for cabin in fuselage.cabins:
                if cabin.tag == self.tag:
                    _, _ = compute_cabin_moment_of_inertia(
                        self, fuselage, center_of_gravity)
        for wing in vehicle.wings:
            if type(wing) == RCAIDE.Library.Components.Wings.Blended_Wing_Body:
                for cabin in wing.cabins:
                    if cabin.tag == self.tag:
                        _, _ = compute_cabin_moment_of_inertia(
                            self, wing, center_of_gravity)
        return
