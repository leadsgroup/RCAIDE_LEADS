...
def flap_noise_model(Sf, cf,thickness, aoa, deltaf, velocity, M, phi, theta, distance, frequency,segment):
    input_parameters = {
        'h': thickness/3.281,                  # Flap thickness (m)
        'L_f': cf/3.281,                       # Flap chord length (m)
        'alpha': aoa,                          # Angle of attack (rad)
        'sigma_f': deltaf+aoa,                 # Flap sweep angle (rad)
        'gamma_f': deltaf,                     # Flap deployment angle (rad)
        'M': M,                                # Flight Mach number
        'U': velocity/1.944,                   # Flight velocity (m/s)
        ...
        'r': distance/3.281,                                              # Observer distance (m)
        'theta': theta,                                                   # Polar angle (overhead = 90 deg)
        #we don't use phi here because the distance in effect is the same.
    }
