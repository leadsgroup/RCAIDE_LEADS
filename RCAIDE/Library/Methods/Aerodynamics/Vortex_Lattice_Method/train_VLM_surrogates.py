...
def train_model(aerodynamics,Mach, vehicle): 
    ...
    # for control surfaces, subtract influence WITHOUT control surface deflected from coefficients WITH control surface
    # deflected. Every control surface is treated identically here (rather than branching on type) since deflecting
    # any control surface (including compound surfaces such as flaperons, elevons, or ruddervators) couples into
    # every force/moment axis, not just the subset a surface is nominally associated with.
...
def train_trasonic_model(aerodynamics, training_subsonic,training_supersonic,sub_Mach, sup_Mach, vehicle): 
    ...
    # for control surfaces, subtract influence WITHOUT control surface deflected from coefficients WITH control surface
    # deflected — treated uniformly for every registered type (see control_surface_registry.py) instead of branching
    # on class, since deflecting any control surface couples into every force/moment axis.
