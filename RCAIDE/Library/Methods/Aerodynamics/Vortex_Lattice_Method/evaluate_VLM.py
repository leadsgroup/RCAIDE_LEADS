...
def evaluate_surrogate(state,settings,vehicle):
    ...
    # -----------------------------------------------------------------------------------------------------------------------
    # Every control surface is handled identically here (see control_surface_registry.py) instead of branching on
    # type, since deflecting any control surface (including compound surfaces such as flaperons, elevons, or
    # ruddervators) couples into every force/moment axis, not just the subset a surface is nominally associated
    # with. Induced drag must increase regardless of which direction a surface deflects (it cannot reduce total
    # induced drag), so unlike the other coefficients its contribution is added as an absolute value rather than
    # the signed linear term used for Y/L/N/M/lift.
    # A compound surface (Flaperon, Elevon, Ruddervator) contributes two rows here that share
    # one `name` conditions block (one per channel it responds to), so that block's diagnostic
    # coefficients are reset to zero once per call and accumulated with += across its rows,
    # rather than overwritten — the vehicle totals above were already correctly additive.
    touched_names = set()
...
def evaluate_no_surrogate(state,settings,vehicle):
    ...
    # every control surface is handled identically here (see control_surface_registry.py) instead of
    # branching on type, since deflecting any control surface couples into every force/moment axis
    # (this also fixes a copy-paste bug in the former Flap/Slat branches that clobbered Clift_delta_f/s
    # with the drag derivative instead of storing Cdrag_delta_f/s).
