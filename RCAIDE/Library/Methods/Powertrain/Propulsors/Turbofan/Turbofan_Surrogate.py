...
class Turbofan_Surrogate(Data):
    ...
    A caller that already knows the real commanded variable for every RC=0
    row -- e.g. a deck generated by sweeping a live off-design solver's own
    Tt4-fraction, rather than an opaque real-world test-stand deck -- should
    not have that real value discarded in favor of the rank-based guess: pass
    it as an explicit "throttle_fraction" column (via `deck.throttle_fraction`
    with `build(deck=...)`, or a "throttle_fraction" column on `dataframe=`).
    When present with no missing values across every RC=0 row, build() uses
    it as-is and skips the rank-based derivation entirely. This matters
    downstream too: query()'s `throttle` argument for RC=0 is meaningful only
    if it is expressed on the same scale the deck's own throttle_fraction
    used to build the interpolator -- a caller that later queries with a real
    physical throttle/Tt4-fraction must have built the deck with real
    throttle_fraction values, not rank-based ones, or the two scales won't
    agree (confirmed to otherwise produce non-monotonic, sign-flipping
    thrust/fuel-flow -- see aircraft_engine_mission_test.py's `idle_fallback`
    investigation in RESEARCH/22_ATI/Engine_Validation/).
    ...
    def deck_to_dataframe(deck):
        ...
        If `deck` also carries a `throttle_fraction` field (1-D array, same
        length as the others), it is passed through as a "throttle_fraction"
        column -- see THROTTLE AXIS in the class docstring for when a caller
        should supply this (a deck built from known commanded values, not an
        opaque real-world deck)."""
        df = pd.DataFrame({
        ...
        throttle_fraction = getattr(deck, 'throttle_fraction', None)
        if throttle_fraction is not None:
            df["throttle_fraction"] = throttle_fraction
        return df
    ...
    def build(self, deck_path=None, sheet_name=None, dataframe=None, deck=None, save_path=None):
        ...
        # RC=0 throttle axis -- see THROTTLE_AXIS in the class docstring. A caller that
        # supplied a real "throttle_fraction" column with no gaps across every RC=0 row
        # (e.g. a deck built from known Tt4-fraction values) keeps it as-is; otherwise
        # rank (not row position) within each (altitude, Mach, ISA) group, descending
        # FN, is mapped to 1.0 (highest thrust in the group) .. 0.0 (lowest) as before.
        is_rc0 = df["RC"] == 0
        if is_rc0.any():
            has_real_throttle_fraction = ("throttle_fraction" in df.columns
                                           and not df.loc[is_rc0, "throttle_fraction"].isna().any())
            if not has_real_throttle_fraction:
