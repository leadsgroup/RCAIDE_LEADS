# RCAIDE/Library/Methods/Powertrain/Propulsors/Turboprop/generate_turboprop_deck.py
#
#
...
from RCAIDE.Library.Methods.Powertrain.Propulsors.Turboprop.design_turboprop_offdesign_matching import (
...
# ----------------------------------------------------------------------------------------------------------------------
#  generate_turboprop_deck
# ----------------------------------------------------------------------------------------------------------------------
def generate_turboprop_deck(turboprop, altitude_range, mach_range, combustor_exit_temperature=None,
                                       isa_deviation=0.0, rating_code=0, allow_unconverged_fallback=False):
    """
    Runs `solve_turboprop_offdesign_robust` across a grid of (altitude, Mach)
    ...
    rationale); this is the turboprop-solver equivalent, feeding the exact
    ...
    turboprop : RCAIDE.Library.Components.Powertrain.Propulsors.Turboprop
        An already-designed turboprop (`design_turboprop(turboprop)` already called).
    ...
    combustor_exit_temperature : float, optional
        ...
        throttle setting. Defaults to the turboprop's own design value.
    ...
    allow_unconverged_fallback : bool, optional
        Passed through to `solve_turboprop_offdesign_robust`. See
    ...
    Notes
    -----
    This deck carries only `thrust_N` (the full `Ccore + Cprop` work-
    coefficient thrust, see `Turboprop_OffDesign_Matching.py`) -- shaft
    power delivered to the propeller (`result.power`) is not part of
    `Turbofan_Surrogate`'s schema and is not carried through here.

    Feed this straight into `Turbofan_Surrogate.build()` via its `deck=`
    argument, exactly as `generate_turbofan_deck`'s own docstring
    shows:

        deck = generate_turboprop_deck(turboprop, altitude_range, mach_range)
        turboprop.surrogate = Turbofan_Surrogate().build(deck=deck)

    `Turboprop` does not currently dispatch on a `.surrogate` attribute the
    way `Turbofan` does -- this deck is meant for `turboprop.offdesign_
