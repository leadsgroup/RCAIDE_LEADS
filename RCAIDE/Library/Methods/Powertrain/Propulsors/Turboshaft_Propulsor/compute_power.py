...
def compute_power(turboshaft,turboshaft_conditions,conditions):
    ...
    tau_tL                                     = tau_t/tau_tH                                                                                            
    x                                          = tau_t*tau_r*tau_c                                                                                     
    C_shaft                                    = tau_lambda*(1 - tau_t) - tau_r*(tau_c - 1)                                                           
 
    #Computing Specifc Thrust 
    Tsp                                        = a0*(((2/(gamma - 1))*(tau_lambda/(tau_r*tau_c))*(tau_r*tau_c*tau_t - 1))**eta_c - M0)                
     
    #computing the core mass flow               
    m_dot_air                                  = m_dot_compressor*np.sqrt(Tref/total_temperature_reference)*(total_pressure_reference/Pref)           
