...
def free_wake(rotor, wake_inputs, conditions):
    ...
    def compute_V_ind(field_grid, source_grid):
        ...
        j are column j of every blade's grid, matched to that query's own instant. J columns
        are batched into a single biot_savart_velocity_induction call per cp, using that
        function's leading-batch-dim support: each batch slice j only interacts with its own
        column's sources -- same block-diagonal sourcing as the old per-j loop, just evaluated
        as one larger vectorized op.

        ctrl_pts is deliberately NOT also batched into that same call: for real wake sizes
        (e.g. Electric_Twin_Otter's propeller, n_turns=5 at dpsi=15deg -> N_wake=121, J=24,
        B=3 -> K_wake alone is ~1.2 GB at ctrl_pts=16, with several same-shaped temporaries
        alive at once inside biot_savart_velocity_induction), fully batching ctrl_pts*J into
        one tensor turned out to be memory-bandwidth/allocation-bound rather than faster --
        confirmed by a real run converging in only 6-7 outer iterations yet still being very
        slow. Keeping cp as a Python loop bounds each call's memory footprint to O(J) instead
        of O(ctrl_pts*J), which is the actual win here, not merely fewer Python calls.
        """
        V_ind = np.zeros_like(field_grid)
        for cp in range(ctrl_pts):
            # (B, J, N_wake+1, 3) -> (J, B*(N_wake+1), 3), same B-major flattening per column
            # as the old P = field_grid[cp, :, j, :, :].reshape(...) call.
            P_all = field_grid[cp].transpose(1, 0, 2, 3).reshape(J, B*(N_wake+1), 3)

            src_all   = source_grid[cp].transpose(1, 0, 2, 3)          # (J, B, N_wake+1, 3)
            r_w_start = src_all[:, :, :-1, :].reshape(J, B*N_wake, 3)
            r_w_end   = src_all[:, :,  1:, :].reshape(J, B*N_wake, 3)

            K_wake = biot_savart_velocity_induction(
                P_all, r_w_start, r_w_end, rcvf_flat[cp], vc_correction)     # (J, M, N_wake_tot, 3)
            v_wake = np.einsum('jmnk,n->jmk', K_wake, Gamma_w_flat[cp])      # (J, M, 3)

            K_blade = biot_savart_velocity_induction(
                P_all, A_blade_all[cp], B_blade_all[cp], rcb_flat[cp], vc_correction)
            v_blade = np.einsum('jmnk,n->jmk', K_blade, Gamma_b_flat[cp])    # (J, M, 3)

            v_sum = (v_blade + v_wake).reshape(J, B, N_wake+1, 3).transpose(1, 0, 2, 3)
            V_ind[cp] = np.where(CW[cp], -v_sum, v_sum)
        return V_ind
