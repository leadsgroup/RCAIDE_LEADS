...
def append_reservoir_conditions(reservoir,segment,coolant_line):
    ...
    segment.state.conditions.energy.distributors[coolant_line.tag][reservoir.tag]                                 = Conditions()
    segment.state.conditions.energy.distributors[coolant_line.tag][reservoir.tag].coolant_temperature             = atmo_data.temperature[0,0]* ones_row(1)
    return

def append_reservoir_segment_conditions(reservoir,segment,coolant_line):
    ...
    reservoir_conditions = segment.state.conditions.energy.distributors[coolant_line.tag][reservoir.tag]
    if segment.state.initials:
        reservoir_initials                                   = segment.state.initials.conditions.energy.distributors[coolant_line.tag][reservoir.tag]
