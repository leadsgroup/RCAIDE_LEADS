...
#  METHOD
# ----------------------------------------------------------------------------------------------------------------------   
def compute_bwb_aft_integral_tank_volume(fuel_tank, wing,_):
    ...
    # Non-dimensional (per-unit-mass) MOI from geometry alone (trimesh assumes
    # density = 1 by default, so moment_inertia/volume is density-independent)
    I_non_dim = tank_mesh.moment_inertia / tank_mesh.volume
    cg_x      = centroid[0]
    cg_y      = 0
    cg_z      = centroid[2]
