...
def update_p30b_cell_age(battery,segment, battery_conditions, increment_battery_age_by_one_day):  
    ...
    # #initial_conditions = battery_conditions.cell.model_conditions #error in regression?
    ...
    # model = battery_module.model.set_initial_conditions_from(pybamm_condition, inplace = False) #pybamm_condition will need to be updated
    # experiment = pybamm.Experiment( ["Discharge at "+ str(I_cell[t_idx]) + "A for " + str(delta_t) + " seconds"] ) 
    # simulation = pybamm.Simulation(model, experiment = experiment, parameter_values = battery_module.model.battery_parameters) 
    # pybamm_condition = simulation.solve(initial_soc = SOC_cell) 
