...
# update_nmc_cell_age
# ----------------------------------------------------------------------------------------------------------------------  
def update_nmc_cell_age(module,battery, segment, increment_battery_age_by_one_day):  
    """
    ...
    -------
    None
        This function modifies the module_conditions object in-place.
    ...
    SOC               = module_conditions.cell.state_of_charge
    V_ul              = module_conditions.voltage_under_load/n_series
    t                 = module_conditions.cell.cycle_in_day         
    Q_prior           = module_conditions.cell.charge_throughput[-1,0] 
    Temp              = np.mean(module_conditions.cell.temperature) 
    ...
    module_conditions.cell.capacity_fade_factor     = np.minimum(E_fade_factor,module_conditions.cell.capacity_fade_factor)
    module_conditions.cell.resistance_growth_factor = np.maximum(R_growth_factor,module_conditions.cell.resistance_growth_factor)
    
    if increment_battery_age_by_one_day:
        module_conditions.cell.cycle_in_day += 1 # update battery age by one day 
