    ...
    **Theory**
    ...
    if ground_seg_flag:
        ...
        if  segment.flight_dynamics.moment_x:
            segment.state.residuals.mission.moment_x[:,0] = MT_w[:,0]/MOI_Ixx[:,0] - ang_acc_w[:,0]   
        if  segment.flight_dynamics.moment_y:
            segment.state.residuals.mission.moment_y[:,0] = MT_w[:,1]/MOI_Iyy[:,0] - ang_acc_w[:,1]   
        if  segment.flight_dynamics.moment_z:
            segment.state.residuals.mission.moment_z[:,0] = MT_w[:,2]/MOI_Izz[:,0] - ang_acc_w[:,2]
    return
